...
from ..parallel_3d._utils import get_input_parallel_mode, get_output_parallel_mode, get_weight_parallel_mode
...
class CrossEntropyLoss3D(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        """
        targets = split_batch_3d(
            targets, input_parallel_mode=self.input_parallel_mode, weight_parallel_mode=self.weight_parallel_mode
        )
...
class _VocabParallelCrossEntropy3D(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, output_parallel_mode):
        ...
        dist.all_reduce(logits_max, op=torch.distributed.ReduceOp.MAX, group=output_parallel_mode.group)
        # Subtract the maximum value.
        logits = logits - logits_max.unsqueeze(dim=-1)

        vocab_size_per_partition = logits.size()[-1]
        rank = output_parallel_mode.local_rank
        ...
        dist.all_reduce(predicted_logits, group=output_parallel_mode.group)

        # Loss = log(sum(exp(logits))) - predicted-logit.
        exp_logits = torch.exp(logits)
        sum_exp_logits = exp_logits.sum(dim=-1)
        dist.all_reduce(sum_exp_logits, group=output_parallel_mode.group)
...
class VocabParallelCrossEntropyLoss3D(nn.Module):
